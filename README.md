...
[![Release](https://img.shields.io/github/v/release/liubarryteb12/dsh-pkos-skill)](https://github.com/liubarryteb12/dsh-pkos-skill/releases)
[![Upstream](https://img.shields.io/badge/upstream-hermes--pkos--skill%20v5.5.2-blue)](https://github.com/liubarryteb12/hermes-pkos-skill)
[![Target](https://img.shields.io/badge/target-DeepSeek%20Harness%20(DSH)-orange)]()
[![Units](https://img.shields.io/badge/capability_units-33-green)]()
...
> **这是 [hermes-pkos-skill](https://github.com/liubarryteb12/hermes-pkos-skill) 的 DeepSeek Harness (DSH) 适配发行版。**
> 完整套件本体在此仓库独立分发：目录结构、契约、脚本与上游保持同版本（v5.5.2），入口 SKILL.md 与路径解析已按 DSH 的技能加载方式适配（`.agents/skills/dsh-pkos-skill/`）。上游更新后本仓库跟随发版；功能需求请到上游仓库提 issue。
...
它由 33 个小工人组成（我们叫它们"单元"）。每个工人只干一件事，干得又快又好。有的负责收快递，有的负责贴标签，有的负责当质检员。它们手拉手排成一条流水线，你的资料从一头进去，礼物从另一头出来。
...
### 第 0 步：对话式初始化（v5.5.0 新增）

不用改任何代码。装好后跟 agent 说「初始化 PKOS」，它会问你三句话：
1. **知识库放哪？**（已有 Obsidian 库直接给路径；没有就让它新建，自动搭好目录骨架）
2. **出稿产物放哪？**（workspace 落点）
3. **收件箱在哪？**（默认库内 `_PKOS/INBOX`）

答完自动生成 `config.json`，所有小工人按这个地址干活。LLM 网关走 Hermes 自身配置，不用管。赶时间的可以跳过——不初始化时系统用默认路径。
