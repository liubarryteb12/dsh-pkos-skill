...
> - **模型通道（09-14 网关实测更新，0.2.2）**：4 号 key（公益组）现仅 agnes-2.5-flash 一条通道——target/planner 默认走 HERMES_CUSTOM_4_API_KEY；executor 默认 sensenova-6.8-flash-lite，**其通道已迁至 9 号 key**（HERMES_CUSTOM_9_API_KEY，脚本 DEFAULT_EXECUTOR_KEY_ENV 已对齐）。8 号 key（国模组）有 deepseek-v4.1-flash/kimi-k3/glm-5.2/hy3/deepseek-v4-flash 五条。qwen3.8-flash 已下线。glm-5.3-flash thinking 禁用（长输出被思考吃光 token 返回空）。
