...
description: "Use when the user wants PKOS knowledge-pipeline work from inside DSH (DeepSeek Harness) — intake triage, ingest, lint, audit, index, timeline, exit products (comic / article / PPT / HTML / GZH publish), scorecard, topic & trend, image prompts. Chinese triggers: 整理知识库 / 消化收件箱 / 体检 / 跑 lint / 建索引 / 时间线 / 漫画 / 公众号文章 / PPT / 打分 / 选题 / 热点 / 题词. Self-contained port of the PKOS suite — dispatch table and guardrails below; per-unit contracts live in each pkos-<unit>/SKILL.md."
...
本目录是 hermes-pkos-skill 的 **DSH 适配发行版**（v5.5.2）：内容 = 主套件核心代码，ROOT/触发词已按 DSH 加载方式适配；上游与发布仓库见 README。隐私约定：本发行版已做消毒（网关 IP→`<LLM_GATEWAY_HOST>`、本机路径→占位符），改动后发布前必须重跑消毒与扫描。
...
- 套件根（下文 ROOT）：DSH 技能目录 `.agents/skills/dsh-pkos-skill`（DSH 会话内相对仓库根解析；独立使用时即本 SKILL.md 所在目录）。
...
cd ".agents/skills/dsh-pkos-skill"   # DSH 仓库根相对路径；独立 clone 时 cd 到本目录
...
- **产出与操作文件分工（09-14 用户裁定）**：产出产品落 `workspace/<类别>/`（由 `scripts/pkos_paths.py` 的 `get_output_root()` 解析，可配置）；操作文件（脚本/状态快照/日志/备份）落套件根 `_ops/`。`_ops/` 与 `cn_debug/` 已进 `.gitignore` 与 `sync_to_main.EXCLUDE_PARTS`——本机资产不发布。

- **改路径类改动必须同步三处**：live（Hermes 加载）→ 主库（`sync_to_main --apply`）→ DSH 副本。只改一处 = 其余仍是旧路径。另需全盘扫兄弟技能与 cron 薄包装脚本。

- **`sync_to_main._norm` 必须做行尾归一**：Windows（`write_text`）写 CRLF、Linux/`echo` 写 LF，同一文件会被 md5 判成「内容漂移」假阳性。比对前统一 `\r\n`/`\r` → `\n`。
...
- **`sync_to_main.py` 有自指豁免**（不自己同步自己）→ 主库版会长期落后；改它之后必须手工把发布版写入主库，否则修复不进发布包。
