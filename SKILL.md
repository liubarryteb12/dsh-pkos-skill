...
> 📍 **产出物位置总表**：[`OUTPUTS-MAP.md`](OUTPUTS-MAP.md) — 所有产线产物落盘位置、知识库内部目录、操作文件 vs 产出产品分开放置说明。找不到东西先看这张表。
...
纪律：①PM 产物进 INBOX 后按既有契约走（validate_entry 不过不得入库）；②判断类问题（该不该做）由 pm-master 转 pm-advisory-board，不直接给结论；③产物落盘仍守 `workspace/` 归位铁律（路径由 `scripts/pkos_paths.py` get_output_root() 统一解析），入库原件才进 INBOX。
